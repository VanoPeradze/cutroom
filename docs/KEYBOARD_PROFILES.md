...
For ordinary mouse editing, **Select** now supports plain dragging across the filmstrip or any linked lane. A click selects an entire kept or removed section; **Remove** excludes footage and **Add to edit** restores it. Drag the ruler to navigate instead. Snap starts off, so selection is not limited to the AI's cut boundaries. These pointer behaviors are CUTROOM's shared workflow, independent of the keyboard preset.
...
- CUTROOM uses one linked edit and removes time from the resulting video. It does not implement Pro Tools' Slip/Shuffle modes, track selection, sample-accurate nudge, clipboard editing, or separate edit/timeline selections. Its A/S trims and Delete/Backspace therefore close gaps, and comma/period move the playhead **one video frame**. The shortcut help marks these adaptations.
