...
This is a private test build of CUTROOM AI 5.6.1. It runs on the receiving PC and opens in a browser. The tester does not need Codex, a ChatGPT account, an API key, or a development environment. This ZIP requires first-run setup; it is not an offline, self-contained executable. A successful installation on a clean PC has not been verified by these instructions.
...
- **AI models:** none are bundled. The supplied configuration has `download_models_on_setup: false`. A missing Story AI model requires a separate named download and confirmation in the app; it can require several GB. The selected Whisper speech model downloads when transcription first needs it. Lite/Balanced/Quality use Base/Small/Turbo respectively, with possible language-specific models. Stay online for these first downloads. Switching profiles can require another model download.
